...
from sqlalchemy import ForeignKey, String, create_engine, Integer, DateTime, Boolean, Float, NVARCHAR, func
...
class DATASET(Base):
    ...
    created_on: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    connection_id: Mapped[Optional[int]] = mapped_column(ForeignKey(f"{SCHEMA}.database_connections.id"))
...
class ReportWorkflow(Base):
    ...
    text_output: Mapped[str] = mapped_column(String(4000), nullable=True)
