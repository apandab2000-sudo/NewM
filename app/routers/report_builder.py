...
async def update_report_component(
    ...
    #     # --- TO THIS ---
    # vector_connection = vector_client.get_vector_db() # Get raw connection
    # vector_store_client = VectorStore(vector_connection) 
    # await vector_store_client.ensure_collection(data.dbname)

    v_client = await get_vector_client()
    vector_store_client = VectorStore(v_client)
    await vector_store_client.ensure_collection(data.dbname)
    ...
    if comp_type == "paragraph":
        ...
        if sql_question:
            t2s = Text2SQL(dbname=data.dbname, vector_client=vector_store_client, llm=llm, llm_name=llm_name, question=sql_question, filters="")
        ...
        # # A. SQL Generation
        t2s = Text2SQL(dbname=data.dbname, vector_client=vector_store_client, llm=llm, llm_name=llm_name, question=data.query, filters="")
        resp = await t2s.sql_query_generator_basic()
    ...
    #     }
        ...
        # 2. SQL Fetching
        t2s = Text2SQL(dbname=data.dbname, vector_client=vector_store_client, llm=llm, llm_name=llm_name, question=sql_q, filters="")
        resp = await t2s.sql_query_generator_basic()
...
##########################################################
    ...
    if component_details.component_type == "table":
        if data.component_metadata.sql:
            ...
            table_details = await get_table_from_sql(client_pool, data.dbname, data.component_metadata.sql, row_limit=1000, query_timeout=20)
...
async def generate_full_dashboard(
    ...
    for idx, point in enumerate(storyboard_points):
        ...
        try:
            ...
            # B. Data Fetching
            # table_details = await get_table_from_sql(client_pool, sql_query)
            table_details = await get_table_from_sql(client_pool, payload.dbname, sql_query)
