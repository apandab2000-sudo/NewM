...
def is_proceed_query(query: str) -> bool:
    # Only return true for very short, unambiguous confirmations
    # to avoid accidentally skipping a real business query.
    q = query.lower().strip().rstrip('!.')
    short_confirms = {"ok", "okay", "yes", "yep", "confirm", "proceed", "next"}
    return q in short_confirms
...
async def handle_conversational_step(i_data, llm_name, payload, report_chat, transactions):
    ...
    if current_step == 1:
        ...
        # Only run Intent ID if the query looks like a new business request (longer than 3 words)
        if query and len(query.split()) > 3:
            intent_res = await report_chat.identify_intent(query, get_chat_history(transactions))
            if intent_res.get("intent_identified", True):
                intent_str = intent_res["response"]["intent"]

        # B. KPI VERIFICATION (This is where the LLM senses the "Proceed" sentiment)
        existing_kpis = [s.option if hasattr(s, 'option') else s.get('option') for s in user_selectables if (s.field if hasattr(s, 'field') else s.get('field')) == "kpis" and (s.is_selected if hasattr(s, 'is_selected') else s.get('is_selected'))]
        ...
        # C. TRANSITION JUDGEMENT
        # We move to Step 2 if:
        # 1. The user clicked the 'Confirm' button
        # 2. OR the LLM explicitly set status to "confirmed"
        # 3. OR the Code Gatekeeper caught a simple "yes/ok"
        if payload.confirm or kpi_res.get("status") == "confirmed" or is_proceed_query(query):
            # JUMP TO STEP 2
            return await handle_step_2_logic(i_data, llm_name, payload, report_chat, transactions, merged_kpis, is_jump=True)
        
        # Otherwise, stay in Step 1
...
async def handle_step_2_logic(i_data, llm_name, payload, report_chat, transactions, current_selectables, is_jump=False):
    ...
    # Resolve Confirmation Sentiment from LLM
    # Syncing the keys: your prompt uses 'user_confirmed', some logic uses 'status'
    llm_confirmed = dim_res.get("user_confirmed") is True or dim_res.get("status") == "confirmed" or dim_res.get("response", {}).get("user_confirmed") is True

    # Transition Logic
    if not is_jump:
        if payload.confirm or is_proceed_query(query) or llm_confirmed:
    ...
    # If we didn't move to Step 3, return Step 2 UI
    ...
    # Merge logic for Dimensions and Time Grain
    # Handling nested 'response' key if your prompt returns it that way
    res_data = dim_res.get("response", dim_res)
    dims = merge_selectables("dimensions", current_selectables, res_data.get("dimensions", {}).get("extracted", []), res_data.get("dimensions", {}).get("suggested", []))
    times = merge_selectables("time_grain", current_selectables, res_data.get("time_grain", {}).get("extracted", []), res_data.get("time_grain", {}).get("suggested", []))
...
#     kpis = [s.get('option') if isinstance(s, dict) else s.option for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) == "kpis" and (s.get('is_selected') if isinstance(s, dict) else s.is_selected)]
#     dims = [s.get('option') if isinstance(s, dict) else s.option for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) == "dimensions" and (s.get('is_selected') if isinstance(s, dict) else s.is_selected)]
#     grains = [s.get('option') if isinstance(s, dict) else s.option for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) == "time_grain" and (s.get('is_selected') if isinstance(s, dict) else s.is_selected)]
    
#     context = f"KPIs: {', '.join(kpis)}. Dimensions: {', '.join(dims)}. Time Grains: {', '.join(grains)}."
    
#     # 2. Call Storyboard Generator
#     story_res = await report_chat.generate_storyboard(context)
    
#     # 3. Final Confirmation: If user is in Step 3 and clicks 'Confirm/Generate'
#     if payload.confirm and not is_jump:
#         return {
#             "status": "complete", 
#             "message": "Finalizing configuration and generating your dashboard components.",
#             "final_configuration": payload.selectables, # Contains everything including storyboard points
#             "step": 4
#         }

#     # 4. Map Storyboard to Selectables
#     story_selectables = []
#     for item in story_res.get("storyboard", []):
#         story_selectables.append({
#             "field": "storyboard_point", 
#             "option": item["statement"], # The "Statement" is the main label
#             "is_selected": True, 
#             "metadata": {"type": item["component_type"]} # Store type for later use
#         })
    
#     # Combine with previous selections to maintain full state (KPIs/Dims)
#     previous_state = [s if isinstance(s, dict) else s.model_dump() for s in current_selectables]
#     all_selectables = previous_state + story_selectables
    
#     return await format_step_response(i_data, payload, story_res, all_selectables, 3, llm_name, report_chat)

# async def handle_step_3_logic(i_data, llm_name, payload, report_chat, transactions, current_selectables, is_jump=False):
#     query = (payload.query or "").strip()
    
#     # 1. IMMEDIATE NL CONFIRMATION CHECK
#     # If user types "looks good" or "proceed" or clicks the button, move to Step 4
#     if (is_proceed_query(query) or payload.confirm) and not is_jump:
#         # Move to Step 4 (Final configuration)
#         # We take the selectables from the payload (what the user has checked)
#         final_config = [s if isinstance(s, dict) else s.model_dump() for s in payload.selectables if (s.is_selected if isinstance(s, dict) else s.is_selected)]
        
#         await i_data.add_report_chat_transaction({
#             "chat_id": payload.chat_id,
#             "raw_response": json.dumps({"status": "FINAL_CONFIRMED"}),
#             "step_at": 4,
#             "user_input": f"User confirmed storyboard: {query}",
#             "transaction_output": json.dumps({"final_selectables": final_config})
#         })

#         return {
#             "status": "complete",
#             "question_to_user": "Perfect! I have finalized the configuration. You can now trigger the report generation.",
#             "final_selectables": final_config,
#             "step": 4
#         }

#     # 2. GENERATE OR REFINE STORYBOARD
#     # If the user provides feedback like "Add a graph for X", the LLM will update the list.
#     kpis = [s.get('option') if isinstance(s, dict) else s.option for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) == "kpis" and (s.get('is_selected') if isinstance(s, dict) else s.is_selected)]
#     dims = [s.get('option') if isinstance(s, dict) else s.option for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) == "dimensions" and (s.get('is_selected') if isinstance(s, dict) else s.is_selected)]
#     grains = [s.get('option') if isinstance(s, dict) else s.option for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) == "time_grain" and (s.get('is_selected') if isinstance(s, dict) else s.is_selected)]
    
#     context = f"KPIs: {', '.join(kpis)}. Dimensions: {', '.join(dims)}. Time Grains: {', '.join(grains) if grains else 'Suggest default'}."
    
#     story_res = await report_chat.generate_storyboard(context, query, get_chat_history(transactions))

#     # 3. SECONDARY NL CONFIRMATION (From LLM Status)
#     if story_res.get("status") == "confirmed" and not is_jump:
#         # Same logic as above: Move to Step 4
#         final_config = [s if isinstance(s, dict) else s.model_dump() for s in payload.selectables if (s.is_selected if isinstance(s, dict) else s.is_selected)]
#         return {
#             "status": "complete",
#             "question_to_user": story_res.get("followup_question", "Configuration confirmed."),
#             "final_selectables": final_config,
#             "step": 4
#         }

#     # 4. PREPARE SELECTABLES FOR UI
#     story_selectables = []
#     for item in story_res.get("storyboard", []):
#         story_selectables.append({
#             "field": "storyboard_point", 
#             "option": item["statement"], 
#             "is_selected": True, 
#             "metadata": {"type": item["component_type"]}
#         })
    
#     # Merge with previous state to keep the whole configuration visible
#     previous_state = [s if isinstance(s, dict) else s.model_dump() for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) != "storyboard_point"]
    
#     return await format_step_response(i_data, payload, story_res, previous_state + story_selectables, 3, llm_name, report_chat)

# async def handle_step_3_logic(i_data, llm_name, payload, report_chat, transactions, current_selectables, is_jump=False):
#     query = (payload.query or "").strip()
    
#     # Helper to extract type from a selectable object/dict
#     def get_selectable_type(s_item):
#         # 1. Check metadata (as dict or attribute)
#         meta = s_item.get('metadata') if isinstance(s_item, dict) else getattr(s_item, 'metadata', None)
#         if meta and isinstance(meta, dict) and meta.get('type'):
#             return meta.get('type')
#         # 2. Check for flattened 'type' at root (fallback)
#         root_type = s_item.get('type') if isinstance(s_item, dict) else getattr(s_item, 'type', None)
#         return root_type or "table"

#     # 1. IMMEDIATE NL CONFIRMATION CHECK
#     if (is_proceed_query(query) or payload.confirm) and not is_jump:
#         final_storyboard_output = []
#         for s in (payload.selectables or []):
#             s_field = s.get('field') if isinstance(s, dict) else s.field
#             s_selected = s.get('is_selected') if isinstance(s, dict) else s.is_selected
#             s_option = s.get('option') if isinstance(s, dict) else s.option

#             # Only include selected storyboard points
#             if s_field == "storyboard_point" and s_selected:
#                 final_storyboard_output.append({
#                     "field": s_field,
#                     "option": s_option,
#                     "type": get_selectable_type(s) # <--- Robust extraction
#                 })
...
#             "step_at": 4,
#             "user_input": f"User confirmed storyboard: {query}",
#             "transaction_output": json.dumps({"final_selectables": final_storyboard_output})
#         })

#         return {
#             "status": "complete",
#             "question_to_user": "Perfect! I have finalized the configuration. I am now ready to generate your report.",
#             "final_selectables": final_storyboard_output,
#             "step": 4
#         }
...
#     # 4. PREPARE SELECTABLES FOR UI (Step 3 Output)
#     story_selectables = []
#     for item in story_res.get("storyboard", []):
#         story_selectables.append({
#             "field": "storyboard_point", 
...
async def handle_step_3_logic(i_data, llm_name, payload, report_chat, transactions, current_selectables, is_jump=False):
    ...
    # Helper to extract type from a selectable object/dict robustly
    def get_selectable_type(s_item):
        # 1. Check root level 'type' (This matches our new Step 3 output format)
        root_type = s_item.get('type') if isinstance(s_item, dict) else getattr(s_item, 'type', None)
        if root_type:
            return root_type
        # 2. Fallback to metadata (for safety with older transaction history)
        meta = s_item.get('metadata') if isinstance(s_item, dict) else getattr(s_item, 'metadata', None)
        if meta and isinstance(meta, dict) and meta.get('type'):
            return meta.get('type')
        return "table"

    # 1. IMMEDIATE NL CONFIRMATION CHECK (Transition to Step 4)
    if (is_proceed_query(query) or payload.confirm) and not is_jump:
        final_storyboard_output = []
        for s in (payload.selectables or []):
            s_field = s.get('field') if isinstance(s, dict) else s.field
            ...
            if s_field == "storyboard_point" and s_selected:
                final_storyboard_output.append({
                    "field": s_field,
                    "option": s_option,
                    "is_selected": s_selected, # Included as requested
                    "type": get_selectable_type(s) # Provided at root
                })
    ...
    # 3. SECONDARY NL CONFIRMATION (From LLM Status -> Transition to Step 4)
    if story_res.get("status") == "confirmed" and not is_jump:
        final_storyboard_output = []
        for s in (payload.selectables or []):
            s_field = s.get('field') if isinstance(s, dict) else s.field
    ...
    for item in story_res.get("storyboard", []):
        story_selectables.append({
            ...
            "option": item["statement"], 
            "is_selected": True, 
            "type": item["component_type"] # TYPE DIRECTLY AT ROOT FOR STEP 3
        })
    
    # Merge with previous state (KPIs/Dims) to keep them in context for the UI
    # Note: We use model_dump() to ensure Pydantic objects are converted to dicts for consistency
    previous_state = [s if isinstance(s, dict) else s.model_dump() for s in current_selectables if (s.get('field') if isinstance(s, dict) else s.field) != "storyboard_point"]
...
async def format_step_response(i_data, payload, results, selectables, step_at, llm_name, report_chat):
    followup = results.get("followup_question") or results.get("response", {}).get("followup_question")
    if not followup:
        followup = "I've suggested a few dashboard components based on your metrics. Which ones would you like to include?"

    transaction_output = {"followup_question": followup, "selectables": selectables}
    
    report_transaction_details = await i_data.add_report_chat_transaction({
        "chat_id": payload.chat_id,
        "raw_response": json.dumps(results),
        "step_at": step_at,
        "user_input": json.dumps({"query": payload.query or ""}),
        "transaction_output": json.dumps(transaction_output)
    })

    # FIX: Check for DB failure before tracking usage
    if report_transaction_details is None:
        logger.error(f"DB Insert failed for Step {step_at}. Check column size for NVARCHAR(4000).")
        return {
            "report_transactions_id": None,
            "question_to_user": followup,
