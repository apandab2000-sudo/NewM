...
def get_3d_bar_and_line_chart(df, numerical_columns:list=[], datetime_columns:list=[], categorical_columns:list=[], graph_label=''):
    ...
    if len(numerical_columns) < 2:
        raise ValueError("This graph requires at least two numerical columns")
    n_col_1, n_col_2 = numerical_columns[:2]
    ...
    fig.add_trace(go.Bar(x=temp[c_col], y=temp[n_col_1],
                     name=n_col_1, marker_color="#008567",
                    #  texttemplate="%{y}",
                     texttemplate="%{y:.2s}",
                     text=temp[n_col_1],
                     textposition="auto",
    ...
    fig.add_trace(go.Scatter(x=temp[c_col], y=temp[n_col_2],
                         ...
                         name=n_col_2, marker_color="#0D5265",
                        #  texttemplate="%{y}",
                         texttemplate="%{y:.2s}",
                         text=temp[n_col_2],
...
def get_2d_vertical_bar_chart(df: pd.DataFrame, numerical_columns: list=[], categorical_columns: list=[], datetime_columns: list=[], graph_label=''):
    ...
    #######2) colour change- -06
    # Chart theme colors
    primary_bar_color = "#008567"      # dark green
    highlight_color = "#008567"        # dark teal
    ...
    # All bars sky blue
    bar_colors = [primary_bar_color] * len(df)

    # Optional: highlight the highest value bar in orange
    if len(df) > 0:
        max_idx = df[numerical_columns[0]].idxmax()
        bar_colors[df.index.get_loc(max_idx)] = highlight_color
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=dict(
                text=categorical_columns[0].replace("_", " "),
                font=dict(color=text_color, size=12.8)
            ),
        ...
        yaxis=dict(
            title=dict(
                text=numerical_columns[0].replace("_", " "),
                font=dict(color=text_color, size=12.8)
            ),
            tickfont=dict(color=text_color, size=12),
            showgrid=True,
...
def get_3d_vertical_cat_bars(df: pd.DataFrame, numerical_columns: list=[], datetime_columns: list=[], categorical_columns: list=[], graph_label=''):
    ...
    if len(categorical_columns) < 2:
        raise ValueError("This graph requires at least two categorical columns")
    c_col_1, c_col_2 = categorical_columns[:2]
...
def _prepare_graph_columns(
    ...
    """
    Normalize detected columns before sending them to a graph function.

    Some graph functions require exactly two numerical or categorical columns.
    Passing the complete detected list caused:
        ValueError: too many values to unpack (expected 2)
    """
    numerical_columns = list(numerical_columns or [])
    ...
    two_categorical_graphs = {
        "get_3d_vertical_cat_bars",
        "get_3d_vertical_cat_bars1",
        "get_3d_hori_cat_bars",
        "get_3d_cat_bars",
        "get_3d_sunburst_custom",
    }

    if graph_type in two_numeric_graphs:
        if len(numerical_columns) < 2:
            raise ValueError(
                f"{graph_type} requires at least 2 numerical columns; "
                f"received {len(numerical_columns)}"
            )
        numerical_columns = numerical_columns[:2]

    if graph_type in two_categorical_graphs:
        if len(categorical_columns) < 2:
            raise ValueError(
                f"{graph_type} requires at least 2 categorical columns; "
                f"received {len(categorical_columns)}"
            )
...
def generate_custom_kpi_graphs(user_query: str, df: pd.DataFrame, graph_details=None, python=False):
    ...
    fig = None
    graph_type = graph_details.get("graph_type")
    if not graph_type:
        raise ValueError("graph_details must contain a non-empty graph_type")
    ...
    print(user_query)
    graph_label = graph_details.get("graph_label", "")
    show_legend = False if graph_details.get("show_legend", "").lower().strip() == "false" else True
    
    numerical_columns, categorical_columns, datetime_columns = _prepare_graph_columns(
    ...
    graph_function = globals().get(graph_type)
    if graph_function is None or not callable(graph_function):
    ...
    if graphs_list:
        for i in range(len(graphs_list)):
            ...
            # fig.update_layout(
            #     template="simple_white",
            #     margin=dict(l=20, r=20, b=20, t=60, pad=0),
            #     height=350,
            #     font=dict(
            #         family="open sans, Helvetica Neue, Helvetica, Arial, sans-serif"
