...
class GenerateSuggestions:
    ...
    async def get_base_examples(self): # this function must be used to find base exmaples
        pass
    ...
    async def get_suggestions(self):
        suggestions_prompt = self.prompt_getter.get_prompt("suggestions_prompt")
        ...
        self.sys_prompt = suggestions_prompt.format(
            schema=schema_str,
            dialect="mssql",
        ...
        messages.append({"role": "user", "content": f"original Question: {self.question}\nOriginal SQL: {self.sql_query}"})

        results, token_usage = await get_llm_response(self.llm, self.llm_name, messages)
        self.token_usage = token_usage
        return results
    
    async def cache_schema_string_for_working_suggestions(self, working_suggestions: list):
        schema_string = await self.get_schema_string(self.question, self.data_filters)
        if not schema_string:
            raise Exception("No schema string found in cache to save for working suggestions")
        
        for ws in working_suggestions:
            ck = f"{self.dbname}__{ws.user_query}"
            await cache_data(ck, schema_string, 24*3600)
