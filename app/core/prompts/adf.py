...
<BUSINESS_RULES>
  * AOV (Average Order Value) = ROUND(SUM(Revenue)*1.0/NULLIF(SUM(Orders), 0), 2)
  * Units per Order = ROUND(SUM(Units)*1.0/NULLIF(SUM(Orders), 0), 2)
  * Drop Off Rates in makerting funnel:
    1) PLP(Visits) to PDP = ROUND((SUM(Visits) - SUM(PDP_Views))*100.0/NULLIF(SUM(Visits), 0), 2)  
    2) PDP to Cart = ROUND((SUM(PDP_Views) - SUM(Cart_Additions))*100.0/NULLIF(SUM(PDP_Views), 0), 2) 
    3) Cart to Checkout = ROUND((SUM(Cart_Additions) - SUM(Checkout))*100.0/NULLIF(SUM(Cart_Additions), 0), 2)
    4) Checkout to Transaction(Orders) = ROUND((SUM(Checkout) - SUM(Orders))*100.0/NULLIF(SUM(Checkout), 0), 2)
  * Conversion Rate = ROUND(SUM(Orders)*100.0/NULLIF(SUM(Visits), 0), 2)
</BUSINESS_RULES>
